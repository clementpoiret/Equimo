...
class TestConvolutionLayers:
    @pytest.mark.parametrize(
        "cls, kwargs",
        [
            (SingleConvBlock, {"in_channels": IN_CHANNELS, "out_channels": OUT_CHANNELS}),
            (DoubleConvBlock, {"dim": IN_CHANNELS}),
            (Stem, {"in_channels": 3, "embed_dim": IN_CHANNELS, "img_size": 32, "patch_size": 4}),
            ...
            (UIB, {"in_channels": IN_CHANNELS, "out_channels": OUT_CHANNELS, "start_dw_kernel_size": 3, "middle_dw_kernel_size": 3}),
            (IFormerStem, {"in_channels": 3, "out_channels": IN_CHANNELS}),
            (IFormerBlock, {"dim": IN_CHANNELS}),
            (GenericGhostModule, {"in_channels": IN_CHANNELS, "out_channels": OUT_CHANNELS}),
            (GhostBottleneck, {"in_channels": IN_CHANNELS, "mid_channels": IN_CHANNELS * 2, "out_channels": OUT_CHANNELS}),
